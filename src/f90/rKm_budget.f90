...
real                                            ::                             &
  ...
  UPx_SO30, VPy_SO30,                                                          &
  ddxPU_SO30, ddyPV_SO30, ddzPW_SO30, cPKm_j, VPy_j, PVy_j,                    &
...
write(*,*) 'test'
allocate( cPKm_new(imt,jmt,km), WVEL(imt,jmt,km),                              &
...
!                 *rho0*1.0E-12
 write(*,*) ' '
  ...
  call vol_int_part(1,n_30S,1,imt,  jmt,km,  PD,TAREA,dz,FBC,   PD_NO30)
  call vol_int_part(1,n_30S,1,imt,  jmt,km,WVEL,TAREA,dz,FBC, WVEL_NO30)
...
201 FORMAT (3(E15.7,","),E15.7)
    open (4,file=trim('../../results/rKm_budget/lat_dependence_'//yr),         &
          form='formatted')
    write(4,200) 'j','cPKm_j','VPy_j','PVy_j'
    cPKm_j = 0.0
    VPy_j  = 0.0
    PVy_j  = 0.0
    do j=1,jmt
