...
&
                   ...
                   * )   &
                     ...
                     + FIELD_k(i  ,j-1) * UAREA(i  ,j-1) * DZU_k(i
,j-1)   &
...
do k = 1,km
  call
uu2tt(imt,jmt,k,DZT(:,:,k),DZU(:,:,k),TAREA,UAREA,TUU_FIELD(:,:,k),WORK)
  TTT_FIELD(:,:,k) = WORK
enddo
...
!===============================================================================
subroutine
uu2tt_scalar(imt,jmt,k,DZT_k,DZU_k,TAREA,UAREA,FIELD_k,NEW_FIELD_k)
!
...
real,    dimension(imt,jmt), intent(in)  :: FIELD_k     ! (T)UU
real,    dimension(imt,jmt), intent(out) :: NEW_FIELD_k ! (T)TT
real :: A, B ! surface integral, used for comparing old/new interp.
method
...
do k = 1,km
  call
uu2tt_scalar(imt,jmt,k,DZT(:,:,k),DZU(:,:,k),TAREA,UAREA,TUU_FIELD(:,:,k),WORK)
  TTT_FIELD(:,:,k) = WORK
enddo
