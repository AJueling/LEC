...
!-----------------------------------------------------------------------
   ...
   integer (POP_i4) :: &
      ...
      tavg_UW,           &! zonal velocity times W velocity                           BY_JAN
      tavg_VW,           &! merid velocity times W velocity                           BY_JAN
      tavg_PV,           &! potential vorticity
      tavg_Q,            &! z-derivative of pot density
!      tavg_PD,           &! potential density 
      tavg_PD,           &! potential density minus rho_sw                            BY_JAN adapted
      tavg_PD2,          &!(potential density minus rho_sw) squared                   BY_JAN
      tavg_RHO2,         &!( in-situ  density minus rho_sw) squared                   BY_JAN
!      tavg_RHOU,         &! pot density times U velocity
!      tavg_RHOV,         &! pot density times V velocity
      tavg_PDU,          &!(pot density minus rho_sw) times U velocity                BY_JAN adapted
      tavg_PDV,          &!(pot density minus rho_sw) times V velocity                BY_JAN adapted
      tavg_PDW,          &!(pot density minus rho_sw) times W velocity                BY_JAN
      tavg_RHOU,         &!(in-situ density minus rho_sw) times U velocity            BY_JAN
      tavg_RHOV,         &!(in-situ density minus rho_sw) times V velocity            BY_JAN
      tavg_RHOW,         &!(in-situ density minus rho_sw) times W velocity            BY_JAN
...
!      tavg_WRHO,         &! pot density   flux through top   face
      tavg_UPD,          &!(pot density minus rho_sw) flux through east  face         BY_JAN adapted
      tavg_VPD,          &!(pot density minus rho_sw) flux through north face         BY_JAN adapted
      tavg_WPD,          &!(pot density minus rho_sw) flux through top   face         BY_JAN adapted
      tavg_URHO,         &!(in-situ density minus rho_sw) flux through east  face     BY_JAN
      tavg_VRHO,         &!(in-situ density minus rho_sw) flux through north face     BY_JAN
      tavg_WRHO,         &!(in-situ density minus rho_sw) flux through top   face     BY_JAN
...
!                          units='cm/s^2', grid_loc='3321')                            !BY_JAN bug in grid_loc???
                          units='cm/s^2', grid_loc='3211')                             !BY_JAN adapted

   call define_tavg_field(tavg_VNU,'VNU',3,                            &
                          long_name='North Flux of Zonal Momentum',    &
!                          units='cm/s^2', grid_loc='3231')                            !BY_JAN bug in grid_loc???
                          units='cm/s^2', grid_loc='3121')                             !BY_JAN adapted
...
!                          long_name='Potential Density Ref to Surface',&
                          long_name='Potential Density Ref to Surface minus rho_sw',&              !BY_JAN adapted
                          units='g/cm^3', grid_loc='3111')
!BY_JAN begin
   call define_tavg_field(tavg_PD2,'PD2',3,                            &
                          long_name='(Potential Density Ref to Surface minus rho_sw) Squared',   &
                          units='g^2/cm^6', grid_loc='3111')

   call define_tavg_field(tavg_RHO2,'RHO2',3,                          &
                          long_name='(In-situ Density minus rho_sw) Squared',     &
                          units='g^2/cm^6', grid_loc='3111')

   call define_tavg_field(tavg_UW,'UW',3,                              &
                          long_name='Zonal velocity times vertical velocity',       &
                          units='cm^2/s^2', grid_loc='3111')

   call define_tavg_field(tavg_VW,'VW',3,                              &
                          long_name='Meridional velocity times vertical velocity',  &
                          units='cm^2/s^2', grid_loc='3111')

   call define_tavg_field(tavg_RHOU,'RHOU',3,                                       &
                          long_name='U times (in-situ density minus rho_sw)',       &
                          units='g/cm^2/s', grid_loc='3111')

   call define_tavg_field(tavg_RHOV,'RHOV',3,                                       &
                          long_name='V times (in-situ density minus rho_sw)',       &
                          units='g/cm^2/s', grid_loc='3111')

   call define_tavg_field(tavg_RHOW,'RHOW',3,                                       &
                          long_name='W times (in-situ density minus rho_sw)',       &
                          units='g/cm^2/s', grid_loc='3111')

   call define_tavg_field(tavg_URHO,'URHO',3,                                                  &
                          long_name='flux of (in-situ density minus rho_sw) across east face', &
                          units='g/cm^3/s', grid_loc='3211')

   call define_tavg_field(tavg_VRHO,'VRHO',3,                                                   &
                          long_name='flux of (in-situ density minus rho_sw) across north face', &
                          units='g/cm^3/s', grid_loc='3121')

   call define_tavg_field(tavg_WRHO,'WRHO',3,                                                 &
                          long_name='flux of (in-situ density minus rho_sw) across top face', &
                          units='g/cm^3/s', grid_loc='3112')

   call define_tavg_field(tavg_PDW,'PDW',3,                                           &
                          long_name='W times (potential density minus rho_sw)',       &
                          units='g/cm^2/s', grid_loc='3111')
!BY_JAN end
!   call define_tavg_field(tavg_RHOU,'RHOU',3,                                         &
   call define_tavg_field(tavg_PDU,'PDU',3,                                         &              !BY_JAN
!                          long_name='U times potential density',       &
                          long_name='U times (potential density minus rho_sw)',       &            !BY_JAN adapted
                          units='g/cm^2/s', grid_loc='3111')

!   call define_tavg_field(tavg_RHOV,'RHOV',3,                                         &
   call define_tavg_field(tavg_PDV,'PDV',3,                                         &              !BY_JAN
!                          long_name='V times potential density',       &
                          long_name='V times (potential density minus rho_sw)',       &            !BY_JAN adapted
                          units='g/cm^2/s', grid_loc='3111')

!   call define_tavg_field(tavg_URHO,'URHO',3,                          &
   call define_tavg_field(tavg_UPD,'UPD',3,                          &                             !BY_JAN
!                     long_name='flux of pot density across east face', &
                          long_name='flux of (pot density minus rho_sw) across east face', &       !BY_JAN adapted
!                          units='g/cm^2/s', grid_loc='3321')                                      !BY_JAN bug in grid_loc and unit???
                          units='g/cm^3/s', grid_loc='3211')                                       !BY_JAN adapted

!   call define_tavg_field(tavg_VRHO,'VRHO',3,                          &
   call define_tavg_field(tavg_VPD,'VPD',3,                          &                             !BY_JAN
!                    long_name='flux of pot density across north face', &
                          long_name='flux of (pot density minus rho_sw) across north face', &      !BY_JAN adapted
!                          units='g/cm^2/s', grid_loc='3231')                                      !BY_JAN bug in grid_loc and unit???
                          units='g/cm^3/s', grid_loc='3121')                                       !BY_JAN adapted

!   call define_tavg_field(tavg_WRHO,'WRHO',3,                          &
   call define_tavg_field(tavg_WPD,'WPD',3,                          &                             !BY_JAN
!                      long_name='flux of pot density across top face', &
                          long_name='flux of (pot density minus rho_sw) across top face', &        !BY_JAN adapted
!                          units='g/cm^2/s', grid_loc='3112')                                      !BY_JAN bug in unit???
                          units='g/cm^3/s', grid_loc='3112')                                       !BY_JAN adapted
   ...
   call define_tavg_field(tavg_UQ,'UQ',3,                              &
                          ...
                          units='g/cm^3/s', grid_loc='3321')                               !BY_JAN bug in grid_loc???

   call define_tavg_field(tavg_VQ,'VQ',3,                              &
                          long_name='flux of Q through north face',    &
                          units='g/cm^3/s', grid_loc='3231')                               !BY_JAN bug in grid_loc???
...
!-----------------------------------------------------------------------
   ...
   real (POP_r8), dimension(nx_block,ny_block,nblocks_clinic) :: & 
      ...
      WTK,                &! vertical velocity at bottom of T box             !BY_JAN: at the top, not at the bottom??
...
!
!-----------------------------------------------------------------------
! BY_JAN: vertical velocity via continuity equation!
...
!-----------------------------------------------------------------------
   ...
   real (POP_r8), dimension(nx_block,ny_block) :: & 
     ...
     RHOIS,            &! in-situ density at k                           BY_JAN
     RHOISM,           &! in-situ density at k-1                         BY_JAN
     WORK,             &! local temp space
...
!BY_JAN begin
          tavg_requested(tavg_PDU) .or.  &
          tavg_requested(tavg_PDV) .or.  &
          tavg_requested(tavg_UPD) .or.  &
          tavg_requested(tavg_VPD) .or.  &
          tavg_requested(tavg_WPD)) then
!BY_JAN end
         ...
         call state(k,1,TRCR(:,:,k,1),                         &
                        ...
                        RHOFULL=RHOK1)
!BY_JAN begin
         RHOK1 = RHOK1-rho_sw     ! other option: RHOOUT=RHOK1
!BY_JAN end
         if (k == 1) then
            RHOK1M = RHOK1
         else
            call state(k-1,1,TRCR(:,:,k,1),                         &
                             ...
                             RHOFULL=RHOK1M)
!BY_JAN begin
            RHOK1M = RHOK1M-rho_sw    ! other option: RHOOUT=RHOK1M
!BY_JAN end
         endif
      endif
...
!BY_JAN begin
      if (tavg_requested(tavg_PD2)) then
         call accumulate_tavg_field(RHOK1*RHOK1,tavg_PD2,bid,k)
      endif
      if (tavg_requested(tavg_PDW)) then
         WORK = RHOK1*(WTK + WTKB)*p5
         call accumulate_tavg_field(WORK,tavg_PDW,bid,k)
      end if

      if (tavg_requested(tavg_UW)) then
         call ugrid_to_tgrid(WORK1,UUU(:,:,k),bid)
         WORK = WORK1*(WTK + WTKB)*p5
         call accumulate_tavg_field(WORK,tavg_UW,bid,k)
      endif
      if (tavg_requested(tavg_VW)) then
         call ugrid_to_tgrid(WORK1,VVV(:,:,k),bid)
         WORK = WORK1*(WTK + WTKB)*p5
         call accumulate_tavg_field(WORK,tavg_VW,bid,k)
      endif

      if (tavg_requested(tavg_RHO2)) then
         call state(k,k,TRCR(:,:,k,1),                      &
                        TRCR(:,:,k,2), this_block,          &
                        RHOFULL=RHOIS)
         RHOIS = RHOIS-rho_sw        ! other option: RHOOUT=RHOIS

         if (k == 1) then
            RHOISM = RHOIS
         else
            call state(k-1,k-1,TRCR(:,:,k,1),               &
                               TRCR(:,:,k,2), this_block,   &
                               RHOFULL=RHOISM)
            RHOISM = RHOISM-rho_sw  ! other option: RHOOUT=RHOISM
         endif

         call accumulate_tavg_field(RHOIS*RHOIS,tavg_RHO2,bid,k)
      endif
      ...
      if (tavg_requested(tavg_RHOW)) then                      
         WORK = RHOIS*(WTK + WTKB)*p5
         call accumulate_tavg_field(WORK,tavg_RHOW,bid,k)
      end if
      if (tavg_requested(tavg_URHO)) then
         WORK = FUE*(RHOIS + eoshift(RHOIS,dim=1,shift=1))
         call accumulate_tavg_field(WORK,tavg_URHO,bid,k)
      endif
      if (tavg_requested(tavg_VRHO)) then
         WORK = FVN*(RHOIS + eoshift(RHOIS,dim=2,shift=1))
         call accumulate_tavg_field(WORK,tavg_VRHO,bid,k)
      endif
      if (tavg_requested(tavg_WRHO)) then
         WORK = dz2r(k)*WTK*(RHOIS + RHOISM)
         call accumulate_tavg_field(WORK,tavg_WRHO,bid,k)
      endif
!BY_JAN end

!      if (tavg_requested(tavg_URHO)) then
      if (tavg_requested(tavg_UPD)) then                      !BY_JAN
         WORK = FUE*(RHOK1 + eoshift(RHOK1,dim=1,shift=1))
         call accumulate_tavg_field(WORK,tavg_UPD,bid,k)      !BY_JAN
!         call accumulate_tavg_field(WORK,tavg_URHO,bid,k)
      endif

!      if (tavg_requested(tavg_VRHO)) then
      if (tavg_requested(tavg_VPD)) then                      !BY_JAN
         WORK = FVN*(RHOK1 + eoshift(RHOK1,dim=2,shift=1))
         call accumulate_tavg_field(WORK,tavg_VPD,bid,k)      !BY_JAN
!         call accumulate_tavg_field(WORK,tavg_VRHO,bid,k)
      endif

!      if (tavg_requested(tavg_WRHO)) then
      if (tavg_requested(tavg_WPD)) then                      !BY_JAN
         WORK = dz2r(k)*WTK*(RHOK1 + RHOK1M)
         call accumulate_tavg_field(WORK,tavg_WPD,bid,k)      !BY_JAN
!         call accumulate_tavg_field(WORK,tavg_WRHO,bid,k)
      endif

!      if (tavg_requested(tavg_RHOU)) then
      if (tavg_requested(tavg_PDU)) then                      !BY_JAN
         call ugrid_to_tgrid(WORK1,UUU(:,:,k),bid)
         WORK = RHOK1*WORK1
         call accumulate_tavg_field(WORK,tavg_PDU,bid,k)      !BY_JAN
!         call accumulate_tavg_field(WORK,tavg_RHOU,bid,k)
      endif

!      if (tavg_requested(tavg_RHOV)) then
      if (tavg_requested(tavg_PDV)) then                      !BY_JAN
         call ugrid_to_tgrid(WORK1,VVV(:,:,k),bid)
         WORK = RHOK1*WORK1
         call accumulate_tavg_field(WORK,tavg_PDV,bid,k)      !BY_JAN
!         call accumulate_tavg_field(WORK,tavg_RHOV,bid,k)
      endif
