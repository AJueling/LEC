...
#gPm_c1_shifted = vertical_integration(LEC_file_c1,nrec_rPm,8,23,DZT)
#gPm_c2_shifted = vertical_integration(LEC_file_c2,nrec_rPm,8,23,DZT)
#gPm_c3_shifted = vertical_integration(LEC_file_c3,nrec_rPm,8,23,DZT)
#gPm_c4_shifted = vertical_integration(LEC_file_c4,nrec_rPm,8,23,DZT)

#gPm_field_shifted = vertical_integration(LEC_file_c1,nrec_rPm,8,23,DZT)
gPm_field = read_binary_2D(LEC_file_c1,imt,jmt,9)
gPm_c1_field_shifted = shift_field(gPm_field,shift)

gPm_field = read_binary_2D(LEC_file_c3,imt,jmt,9)
...
# example
#gPm_int = horizontal_integration(gPm_field_shifted,TAREA,0,imt-1,0,866)
#print('rPm SO30: ', gPm_int/1e9, 'GJ/m')
...
# cs = m.pcolormesh(x,y,gPm_c3_shifted-gPm_c1_shifted,vmin=-4e5,vmax=4e5,cmap='RdBu_r')#,vmin=-0.01,vmax=0.01,shading='flat')
 cs = m.pcolormesh(x,y,gPm_c1_field_shifted,cmap='RdBu_r',vmin=0.,vmax=2000,shading='flat')
...
# cb.set_label(label='colorbar text', fontsize=16)
# plt.legend(fontsize=16)
 f.tight_layout()
 plt.savefig('rPm_c1_level9.png')
...
m.drawmeridians(np.arange(-180.,181.,60.),fontsize=12)
#cs = m.pcolormesh(x,y,gPm_c3_shifted-gPm_c1_shifted,vmin=-4e5,vmax=4e5,cmap='RdBu_r')#,vmin=-0.01,vmax=0.01,shading='flat')
cs = m.pcolormesh(x,y,gPm_c3_field_shifted-gPm_c1_field_shifted,cmap='RdBu_r',vmin=0.,vmax=500,shading='flat')
#plot_rectangle(m,-35,80,-78,-50,'orange','WGKP region')
plot_rectangle(m,-35,80,-78,-50,'orange')
cb = m.colorbar(cs,location='right',pad="1%",format='%.1f')#,ticks=[-.1,0,.1])
#cb.set_label(label='colorbar text', fontsize=16)
#plt.legend(fontsize=16)
f.tight_layout()
plt.savefig('rPm_c3_c1_level9.png')
#plt.savefig('try.eps', format='eps', dpi=100)
plt.show()
plt.clf()
