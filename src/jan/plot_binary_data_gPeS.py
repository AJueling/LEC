...
#gPm_c1_field_shifted = vertical_integration(LEC_file_c1,nrec_cKem,10,25,DZT)
gPm_field = read_binary_2D(LEC_file_c1,imt,jmt,nrec_gPes)
gPm_c1_field_shifted = shift_field(gPm_field,shift)
gPm_field = read_binary_2D(LEC_file_c2,imt,jmt,nrec_gPes)
gPm_c2_field_shifted = shift_field(gPm_field,shift)
gPm_field = read_binary_2D(LEC_file_c3,imt,jmt,nrec_gPes)
gPm_c3_field_shifted = shift_field(gPm_field,shift)
gPm_field = read_binary_2D(LEC_file_c4,imt,jmt,nrec_gPes)
gPm_c4_field_shifted = shift_field(gPm_field,shift)
...
print('gPeS SO30: ', gPm_int/1e9, 'GW')
#exit()
# if( 1==2 ):
...
# m = Basemap(projection='kav7',lon_0=0,resolution='c')  # global projection
 ...
 cs = m.pcolormesh(x,y,gPm_c1_field_shifted*100,cmap='RdBu_r',vmin=-.5,vmax=.5,shading='flat')
...
# plot_rectangle(m,-35,80,-78,-50,'orange','WGKP region')
 ...
 cb.set_label(label='[cW/m^2]', fontsize=16)
# plt.legend(fontsize=16)
 f.tight_layout()
 plt.savefig('gPeS_c1_int9_24.png')
...
# cs = m.pcolormesh(x,y,gPm_c3_shifted-gPm_c1_shifted,vmin=-4e5,vmax=4e5,cmap='RdBu_r')#,vmin=-0.01,vmax=0.01,shading='flat')
 cs = m.pcolormesh(x,y,(gPm_c3_field_shifted - gPm_c1_field_shifted)*1000,cmap='RdBu_r',vmin=-1.,vmax=1.,shading='flat')
# plot_rectangle(m,-35,80,-78,-50,'orange','WGKP region')
 plot_rectangle(m,-35,80,-78,-50,'orange')
 cb = m.colorbar(cs,location='right',pad="1%",format='%.1f')#,ticks=[-.1,0,.1])
 cb.set_label(label='[mW/m^2]', fontsize=16)
# plt.legend(fontsize=16)
 f.tight_layout()
 plt.savefig('gPeS_c3_c1_int9_24.png')
# plt.savefig('try.eps', format='eps', dpi=100)
 plt.show()
 plt.clf()
...
# cs = m.pcolormesh(x,y,(gPm_c2_field_shifted - gPm_c1_field_shifted)*10,cmap='RdBu_r',vmin=-1.,vmax=1.,shading='flat')
 cs = m.pcolormesh(x,y,(gPm_c2_field_shifted - gPm_c1_field_shifted)*1000,cmap='RdBu_r',vmin=-1.,vmax=1.,shading='flat')
# plot_rectangle(m,-35,80,-78,-50,'orange','WGKP region')
 plot_rectangle(m,-35,80,-78,-50,'orange')
 cb = m.colorbar(cs,location='right',pad="1%",format='%.1f')#,ticks=[-.1,0,.1])
...
# plt.legend(fontsize=16)
 f.tight_layout()
 plt.savefig('gPeS_c2_c1_int9_24.png')
# plt.savefig('try.eps', format='eps', dpi=100)
 plt.show()
 plt.clf()
...
# cs = m.pcolormesh(x,y,(gPm_c4_field_shifted - gPm_c1_field_shifted)*10,cmap='RdBu_r',vmin=-1.,vmax=1.,shading='flat')
 cs = m.pcolormesh(x,y,(gPm_c4_field_shifted - gPm_c1_field_shifted)*1000,cmap='RdBu_r',vmin=-1.,vmax=1.,shading='flat')
# plot_rectangle(m,-35,80,-78,-50,'orange','WGKP region')
 plot_rectangle(m,-35,80,-78,-50,'orange')
 cb = m.colorbar(cs,location='right',pad="1%",format='%.1f')#,ticks=[-.1,0,.1])
# cb.set_label(label='[dW/m^2]', fontsize=16)
...
# plt.legend(fontsize=16)
 f.tight_layout()
 plt.savefig('gPeS_c4_c1_int9_24.png')
# plt.savefig('try.eps', format='eps', dpi=100)
 plt.show()
 plt.clf()
