...
# m.drawparallels(np.arange(-90.,91.,30.),labels=[True,False,False,True],fontsize=12) # 
# m.drawmeridians(np.arange(-180.,181.,60.),labels=[False,False,False,True],fontsize=12)
# cs = m.pcolormesh(x,y,np.log(gPm_c1_shifted),cmap='RdBu_r')#,vmin=-1.01,vmax=1.01,shading='flat')
# # use this command after plotting the field:
...
cs = m.pcolormesh(x,y,gPm_field_shifted/10000000.,cmap='RdBu_r',vmin=-0.05,vmax=0.05,shading='flat')
#plot_rectangle(m,-35,80,-78,-50,'orange','WGKP region')
plot_rectangle(m,-35,80,-78,-50,'orange')
cb = m.colorbar(cs,location='right',pad="1%",format='%.1f')#,ticks=[-.1,0,.1])
#cb.set_label(label='colorbar text', fontsize=16)
